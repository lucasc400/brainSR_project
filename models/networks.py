...
def define_G(opt):
    ...
    if which_model == 'sr_resnet':
        netG = G.SRResNetGenerator(input_ngc=opt["input_ngc"], output_ngc=opt["output_ngc"],
                                   ngf=opt["ngf"], ngb=opt["ngb"], norm_type=opt["norm_type"])
    elif which_model == 'sr_gan':
        netG = G.SRGANGenerator(input_ngc=opt["input_ngc"], output_ngc=opt["output_ngc"],
                                ngf=opt["ngf"], ngb=opt["ngb"], norm_type=opt["norm_type"])
    else:
