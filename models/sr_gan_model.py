...
class SRGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.lambda_spatial = opt["train"].get("lambda_spatial") if self.use_spatial_G else 0.0
        ...
        self.lambda_vgg_G = opt['train'].get('lambda_vgg_G') if self.use_vgg_G else 0.0
        if self.use_vgg_G:
        ...
        # For discriminator (D)
        # Adversarial
        self.use_adversarial_D = opt['train'].get('lambda_adversarial_G') is not None and opt['train'].get('lambda_adversarial_D') is not None
        self.lambda_adversarial_G = opt['train'].get('lambda_adversarial_G') if self.use_adversarial_D else 0.0
        self.lambda_adversarial_D = opt['train'].get('lambda_adversarial_D') if self.use_adversarial_D else 0.0
        if self.use_adversarial_D:
            ...
            self.loss_adversarial_D = Loss(opt['train'].get('criterion_adversarial_D'))(opt['train'].get('criterion_adversarial_D'))
            if opt['gpu_ids']:
        ...
        self.weight_decay_D = opt['train'].get('weight_decay_D') if opt['train'].get('weight_decay_D') else 0.0
    ...
    def get_current_losses(self):
        ...
        if self.use_adversarial_D:
            ...
            out_dict['Adv. loss for pos. D'] = self.scaled_loss_adversarial_pos_D.item()
            out_dict['Adv. loss for neg. D'] = self.scaled_loss_adversarial_neg_D.item()
